...
const outputPath = path.join(evidenceDir, `m8-rc-gate-${dateStamp}-${environment}.md`);
...
const content = `# M8 Release Candidate Gate Evidence (${dateStamp})
...
if (status === "fail") {
  console.error(
    `M8 RC gate evidence generated with failure status: ${path.relative(workspaceRoot, outputPath)}`
  );
  process.exit(1);
}
