...
| GET    | `/integrations/messaging/fault-injection/manifest/verify/attempts/retention`                  | Show retention policy plus saturation/trend anomalies with instance ids, triage state, and acknowledgement tracking.       |
| GET    | `/integrations/messaging/fault-injection/manifest/verify/attempts/retention/saturation-trend` | Query saturation trend snapshots with anomaly instance ids, severity, and triage summaries using time-window controls.      |
| POST   | `/integrations/messaging/fault-injection/manifest/verify/attempts/retention/anomalies/{anomalyInstanceId}/triage` | Acknowledge one replay-attempt anomaly instance and append triage notes for operator handoff.                              |
| POST   | `/integrations/messaging/fault-injection/manifest/verify/attempts/retention/apply`            | Apply replay-attempt retention tuning controls, optional prune, and return updated saturation/trend diagnostics.           |
