...
const fileName = `m9-cutover-rehearsal-${dateInput}-${environment}.md`;
...
try {
  execSync("pnpm run pilot:m9:rehearsal:check", {
    ...
    env: {
      ...process.env,
      SKIP_M9_CUTOVER_REHEARSAL_EVIDENCE_CHECK: "1",
    },
  });
} catch (error) {
  ...
  console.error("M9 rehearsal readiness validation failed while generating evidence:");
